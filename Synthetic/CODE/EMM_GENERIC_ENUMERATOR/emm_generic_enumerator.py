...
POSSIBLE_ENUMERATOR_MINIMAL_DESCRIPTION={
    ...
    'themes2':description_minimal_themes2
} 
...
def get_arrdata_from_dataset_values(arr_distinct_values,attributes,votes_attributes,users_attributes,position_attribute):
    ...
    for i,attr in enumerate(attributes):
        ...
        arr_depthmax.append(attr.get('widthmax',None))
...
def generic_enumerators_top_k(dataset,attributes,configuration,user1_scope,user2_scope,votes_attributes,users_attributes,position_attribute,closed=False): #disagreement
    spent=0
...
#     all_visited_dict={}
#     for obj in votes_map_details_array:
#         elem=obj['PROCEDURE_SUBJECT0']
#         if all_visited not in all_visited:
#             all_visited.append(sorted(elem))
#     for i,x in enumerate(all_visited):
#         all_visited_dict[str(i+2471)]=x
#     print all_visited_dict
#     raw_input('....')
    ...
    for p,bitwise_p,stats in enumerator: 
        ...
        index+=1
        if index%100==0:
            print index,nb_quality_measured
...
#                 else :
                arr_data_detailed_append(min_arr)
                arr_refin_detailed_append(min_rein)
...
#             continue
        ...
        returned_mepsStatistics,returned_mepsMeta = extractStatistics_fromdataset_new_update_not_square_vectors(stats, original_mepsMeta, votes_attributes, users_attributes, position_attribute, v_ids, users_ids_set,users1_ids_set,users2_ids_set)
        configuration['stats']=returned_mepsStatistics
        issquare=(users1_ids_set==users2_ids_set)
        #print issquare
        ######################################UPPER BOUND EXPECTED COMPUTING######################################
        
        original_model,pattern_model=compute_models(original_mepwise_similarities,returned_mepsStatistics,comparaison_measure,issquare)
