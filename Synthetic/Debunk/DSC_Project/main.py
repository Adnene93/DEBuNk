	...
	if type(args.performance) is list:
		...
		if args.edf:
			...
			if not args.rwf:
				...
				plotPerf(json_file_path,args.x_axis_attribute,activated=activated,BAR_LOG_SCALE=not args.linear_scale,TIME_LOG_SCALE=not args.linear_scale,plot_bars = not args.do_not_plot_bars, plot_time = not args.do_not_plot_time)
			else:
				sampling_algorithm='DSC+RandomWalk'
				if args.sampling_algorithm is not None and len(args.sampling_algorithm)>0:
					sampling_algorithm=args.sampling_algorithm[0]
