...
elif CONFIGURATION=="Q7": #Sampling EDF
	FONTSIZE = 40
	LEGENDFONTSIZE = 32
	MARKERSIZE = 8
	LINEWIDTH = 7
...
if CONFIGURATION=="Q6": #ECML VS DSC TOPK
	FONTSIZE = 53
	LEGENDFONTSIZE = 45
	MARKERSIZE = 10
	LINEWIDTH = 8
	FIGSIZE=(11.5, 7)#(11.5, 7)
	LOG_SCALE_X=False
	WIDTHOFBARS=0.6
	NB_VALUES_TO_PINCH=100
	XAXISVALUE_REDUCEPRECISION=False
...
if CONFIGURATION=="Q51": #ECML VS DSC
	FONTSIZE = 53
	LEGENDFONTSIZE = 40
	MARKERSIZE = 10
	LINEWIDTH = 8
...
colorByOptBars =  {"DISSENT":"#D64541","CONSENT":"#117733","ITEMSET":"#d7191c","HMT":"#2b83ba","DSC+CLOSED+UB1":"#88CCEE","DSC+CLOSED+UB2":"#2b83ba","debunk":"#2b83ba","dsc":"#d7191c", "DSC+CLOSED" : "#CC6677", "UB1":"red","UB2":"magenta", "DSC":"#DDCC77", "DSC+RandomWalk":"#fdae61","DSC+SamplingPeers+RandomWalk":"#fdae61"}
colorByOptLines =  {"DISSENT":"#D64541","CONSENT":"#117733","ITEMSET":"#d7191c","HMT":"#2b83ba","DSC+CLOSED+UB1":"#88CCEE","DSC+CLOSED+UB2":"#2b83ba","debunk":"#2b83ba","dsc":"#d7191c", "DSC+CLOSED" : "#CC6677", "UB1":"red","UB2":"magenta", "DSC":"#DDCC77", "DSC+RandomWalk":"#fdae61","DSC+SamplingPeers+RandomWalk":"#fdae61"}
...
def plotPerf(testResultFile, var_column, activated = list(optNames.values()), plot_bars = True, plot_time = True,show_legend=True,rotateDegree=0,BAR_LOG_SCALE=False,TIME_LOG_SCALE=False,results_destination=None) :
	PLOT_FIXED=True
	...
	for optName in activated:
		...
		if CONFIGURATION=="Q5" or CONFIGURATION=="Q6":
			...
			execTimeVectorOfVector= np.array([sorted(df[df[var_column]==element]["timespent"]) for element in distinctVarVector])
		...
		if len(distinctVarVectorFixed)>0:
			if plot_bars : 
				if CONFIGURATION=="Q1":
					print ' HEY '
					print nbVisitedVector,[x/float(10**6) for x in nbVisitedVector]
				...
				elif CONFIGURATION=="Q5" or CONFIGURATION=="Q6":
					...
					# raw_input('....')
					linewidth_to_be_used=6.
					bpl=barsAx.boxplot(list(nbVisitiedVectorOfVector),positions=list(distinctVarVectorFixed+offset), widths = barWidth,labels=list(distinctVarVector),meanline=True,showmeans =True,medianprops={'linewidth': linewidth_to_be_used},boxprops={'linewidth': linewidth_to_be_used},whiskerprops={'linewidth': linewidth_to_be_used},capprops={'linewidth': linewidth_to_be_used},flierprops={'linewidth': linewidth_to_be_used})
					set_box_color(bpl,colorByOptBars[optName])
					if CONFIGURATION=="Q6":
						plt.plot([], colorByOptBars[optName], label=LabelOptName,linewidth=linewidth_to_be_used)
			...
			if show_legend : 
				...
				if not plot_time:
					barsAx.legend(loc='upper center', bbox_to_anchor=(0.5, 1.1),ncol=3,fancybox=True,fontsize=LEGENDFONTSIZE, framealpha=0.85)
				else:
	...
	fig.tight_layout()
	exportPath = results_destination if results_destination is not None else exportPath
...
def plotRW(testResultFile, var_column, activated = list(optNames.values()), plot_bars = False, plot_time = True,show_legend=True,rotateDegree=0,BAR_LOG_SCALE=False,TIME_LOG_SCALE=False,sampling_algorithm='DSC+RandomWalk') :
	...
	colors=[ '#abdda4', '#2b83ba', '#fdae61', '#117733', '#999933', '#DDCC77', '#CC6677', '#882255', '#AA4499']
	ind_color=0
	...
	for optName in activated:
		for target in ['precision','recall','f1_score']: 
			...
			if len(distinctVarVectorFixed)>0:
				if plot_bars : 
					barsAx.bar(distinctVarVectorFixed+offset, np.array([x for x in nbVisitedVector]), hatch= hatchTypeByOpt[optName], width = barWidth, align='center', color= colorByOptBars[optName],label=optName,edgecolor=colorByOptEdge[optName],alpha=0.95)
				if plot_time : 
					#timeAx.errorbar(distinctVarVectorFixed, execMeanTimeVector, yerr = execErrorTimeVector,fmt = lineTypeByOpt[optName]+markerByOpt[optName], linewidth=LINEWIDTH+2,markersize=MARKERSIZE,label=optName, color= 'black')
...
def plotHistograms_edf(data,exportpath,activated=[0,1,2,3,4,5,7]):
	mapping_algos_activation={   #(Algo, No_generality,No sigma)
		...
		('Quick-DEBuNk-NoRW',True,False):7,
	}
	# 0 DEBUNK
	# 1 QuickDEBUNK
	# 2 
	...
	mapping={
		...
		'Quick-DEBuNk-NoRW':'DSC+CLOSED',
	}
	id_color=0
	fig, baseAx = plt.subplots(figsize=FIGSIZE) #TIMO
	baseAx.tick_params(axis='x', labelsize=FONTSIZE)
	baseAx.tick_params(axis='y', labelsize=FONTSIZE)
	baseAx.set_xlabel(r'$\varphi$',fontsize=FONTSIZE-5)
	baseAx.set_ylabel(r'Prob.',fontsize=FONTSIZE-5)
	...
	for row in data:
		...
		print colorByOptBars[mapping[algorithm]]

		width=0.01
		x_axis_ticks,nb_hists=histograms(edf,width=width,lim_max=0.6)
		...
		plt.bar(x_axis_ticks,nb_hists,width, label=algorithm,alpha=0.9,color=colorByOptBars[mapping[algorithm]])#colors[id_color])#colorByOptBars[mapping[algorithm]])
		id_color+=1
	# print [x.get_text() for x in baseAx.get_yticklabels()]
	# baseAx.set_yticklabels([r'$'+('%.f'%(float(str(item.get_text()))*100))+'$\%'  for item in baseAx.get_yticklabels()][:])

	vals = baseAx.get_yticks()
	print vals
	baseAx.set_yticklabels([r'$'+str(int(float(x)*100))+'$\%' for x in vals][:-1])
	plt.legend(loc='upper right', fancybox=True, framealpha=0.85,fontsize=35)
	fig.tight_layout()
	plt.savefig(exportpath)
	fig.tight_layout()
...
def plot_bars_vector_many_populations(dict_vectors,exportPath): #dict_vectors={'pop1':v1,'pop2':v2}
	...
	for v in dict_vectors:
		...
		rects1 = ax.bar(ind+offset, vector, width, color=arr_colors[k % 2],alpha=0.95,hatch='//',label=v)
		offset+=width
		k+=1
	...
	ax.set_ylabel('\% outcomes',fontsize=52)
	ax.set_xlabel('scores',fontsize=52)
	...
	ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.05),ncol=3,fancybox=True,fontsize=50, framealpha=0.85)
...
def plot_bars_vector_many_populations_openmedic(dict_vectors,exportPath,order_of_keys=None,title_for_this=' '): #dict_vectors={'pop1':{'a1':v1,'a2':v2},'pop2':v2}
	...
	ax.set_ylabel('Ratio   ',fontsize=52)
	ax.set_xlabel(title_for_this,fontsize=52)
	...
	def autolabel(rects,vector_associated,j=0):
		import humanize
		"""
		Attach a text label above each bar displaying its height
		"""
		...
		for rect in rects:
			...
			# 		j=2.2
			height = rect.get_height()
			ax.text(rect.get_x() + rect.get_width()/2., (1.05+j*0.25)*height,
					humanize.intword(vector_associated[order_of_keys[i]]), #intcomma
					ha='center', va='bottom',fontsize=40)
			i+=1

	autolabel(rects1,v1,j=0.8)
	autolabel(rects2,v2,j=2)
	#ax.legend(loc='lower left', shadow=True, fontsize=35, framealpha=0.7) #'upper left' 'lower right'
	ax.legend(loc='upper center', bbox_to_anchor=(0.73, 0.18),ncol=3,fancybox=True,fontsize=35, framealpha=0.85)
	#ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.05),ncol=3,fancybox=True,fontsize=50, framealpha=0.85)
