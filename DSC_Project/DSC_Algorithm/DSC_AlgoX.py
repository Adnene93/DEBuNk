...
def DSC_Entry_Point(itemsFile,usersFile,reviewsFile,numeric_attrs=[],array_attrs=None,outcome_attrs=None,method_aggregation_outcome='VECTOR_VALUES',itemsScope=[],users_1_Scope=[],users_2_Scope=[],delimiter='\t',
	...
	for interesting_patterns in TOCALL(items_metadata,users_metadata,considered_items_sorted,considered_users_1_sorted,considered_users_2_sorted,all_users_to_items_outcomes,
		...
		if write_results_in_file:
			yield DSC_exiting_point(interesting_patterns,all_users_to_items_outcomes,items_metadata,users_metadata,items_id_attribute,users_id_attribute,results_destination,types_attributes_items,types_attributes_users,TOCALL.stats['attrs_objects'],TOCALL.stats['attrs_users'],vector_of_action,write_results_in_file)
		else:
...
def DSC_exiting_point(interesting_patterns,all_users_to_items_outcomes,items_metadata,users_metadata,items_id_attribute,users_id_attribute,file_destination,types_attributes_items,types_attributes_users,description_attributes_items,description_attributes_users,vector_of_action,write_results_in_file=True):
	...
	for e_u_p,e_u_label,quality,borne_max_quality,e_u_p_ext,e_u_p_ext_bitset,ref_sim,pattern_sim in sorted_interesting_patterns:
		...
		if WRITE_MORE_DETAILS:
			...
			if type_of_data=='EPD':
				...
				writeCSVwithHeader(TO_WRITE_DETAILS_U1,'./DetailedResults/'+str(k).zfill(4)+'_entities'+'.csv',selectedHeader=entities_file_selected_header)
				writeCSVwithHeader([users_metadata[i] for i in e_u_p_ext[1]],'./DetailedResults/'+str(k).zfill(4)+'_g1'+'.csv',selectedHeader=users_file_selected_header)
				writeCSVwithHeader([users_metadata[i] for i in e_u_p_ext[2]],'./DetailedResults/'+str(k).zfill(4)+'_g2'+'.csv',selectedHeader=users_file_selected_header)
				...
				writeCSVwithHeader(g1_outcomes,'./DetailedResults/'+str(k).zfill(4)+'_g1_outcomes'+'.csv',selectedHeader=[items_id_attribute,users_id_attribute,'outcome'])
				writeCSVwithHeader(g2_outcomes,'./DetailedResults/'+str(k).zfill(4)+'_g2_outcomes'+'.csv',selectedHeader=[items_id_attribute,users_id_attribute,'outcome'])
				

				writeCSVwithHeader([{items_id_attribute:e, users_id_attribute:'g1','outcome '+str(vector_of_action):g1_agg_votes[e] if e in g1_agg_votes else '-'} for e in e_u_p_ext[0]],'./DetailedResults/'+str(k).zfill(4)+'_g1_aggregated_outcomes'+'.csv',selectedHeader=[items_id_attribute,users_id_attribute,'outcome '+str(vector_of_action)])
				writeCSVwithHeader([{items_id_attribute:e, users_id_attribute:'g2','outcome '+str(vector_of_action):g2_agg_votes[e] if e in g2_agg_votes else '-'} for e in e_u_p_ext[0]],'./DetailedResults/'+str(k).zfill(4)+'_g2_aggregated_outcomes'+'.csv',selectedHeader=[items_id_attribute,users_id_attribute,'outcome '+str(vector_of_action)])
				...
				if type_of_data=='OPENMEDIC': 
					...
					if DRAW_FIGURES:
						...
						plot_bars_vector_many_populations_openmedic(dictus,'./DetailedResults/'+str(k).zfill(4)+'_patternAndRef'+'.pdf',['Population','*','Context'])
					#raw_input('...')
				...
				if type_of_data in {'MOVIELENS','YELP'}:
					...
					if DRAW_FIGURES:
						...
						plot_bars_vector_many_populations(DICT_VECTOR,'./DetailedResults/'+str(k).zfill(4)+'_u2_agg'+'.pdf')
						...
						plot_bars_vector_many_populations(DICT_VECTOR,'./DetailedResults/'+str(k).zfill(4)+'_u2_agg_referencial'+'.pdf')
		k+=1
	#writeCSVwithHeader(to_write,file_destination,selectedHeader=['ind','context','g1','g2','|subgroup(context)|','|subgroup(g1)|','|subgroup(g2)|','|ratings(c,g1,g2)|','ref_sim','pattern_sim','quality'])
