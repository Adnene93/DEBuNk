...
def enumerator_complex_cbo_init_new_config(dataset,attributes,config_init={},threshold=1,verbose=False,bfs=False,closed=True,do_heuristic=False,heuristic=heuristics[1],initValues=None): #initValues : is a configuration#
#     pr = cProfile.Profile()
#     pr.enable()
	...
	if initValues is None or initValues['config'] is None:
		...
		config={'support':dataset,'flag':True,'indices':set(range(len(dataset))),'allindex':create_index_complex(dataset, attributes),'nb_visited':[0,0,0],'indices_bitset':2**len(dataset)-1,'attributes':attributes}
